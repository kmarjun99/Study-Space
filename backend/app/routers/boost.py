...
async def cancel_boost_request(
    request_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    Cancel/delete a boost request.
    Only INITIATED or PAYMENT_PENDING requests can be cancelled.
    """
    # Get the request
    result = await db.execute(
        select(BoostRequest).where(
            BoostRequest.id == request_id,
            BoostRequest.owner_id == current_user.id
        )
    )
    ...
    # Only allow cancellation of INITIATED or PAYMENT_PENDING requests
    cancellable_statuses = [BoostRequestStatus.INITIATED, BoostRequestStatus.PAYMENT_PENDING]
    if boost_request.status not in cancellable_statuses:
        raise HTTPException(
            status_code=400, 
            detail=f"Cannot cancel request with status: {boost_request.status.value}. Only 'initiated' or 'payment_pending' requests can be cancelled."
        )
    
    # Delete the request
    await db.delete(boost_request)
    await db.commit()
    
    return {"message": "Boost request cancelled successfully", "request_id": request_id}
...
async def mark_request_paid(
    ...
    try:
        ...
        if payment_data.order_id.startswith("order_mock_") or payment_data.order_id.startswith("order_demo_"):
             print(f"DEMO/MOCK BOOST: Skipping signature verification for {payment_data.order_id}")
        else:
             # 1. Verify Signature
            params_dict = {
