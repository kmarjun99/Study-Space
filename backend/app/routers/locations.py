...
async def seed_locations_from_json(
    clear_existing: bool = Query(False, description="If true, delete all existing locations before seeding"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    Seed the locations table from the bundled cities.json file.
    Super Admin only. Safe to call multiple times — skips if data already exists
    unless clear_existing=true.
    """
    if current_user.role != UserRole.SUPER_ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only Super Admin can seed locations"
        )

    if not CITIES_FILE.exists():
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="cities.json not found in the server bundle. Please redeploy."
        )
    ...
    if existing_count > 0 and not clear_existing:
        return SeedResponse(
            ...
            message=f"{existing_count} locations are already seeded. Pass clear_existing=true to re-seed."
        )
    ...
    return SeedResponse(
        ...
        message=f"Successfully seeded {inserted} locations from cities.json."
    )
