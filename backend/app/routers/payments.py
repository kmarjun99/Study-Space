...
from app.services.invoice_pdf_service import render_credit_note
...
async def verify_payment(
    ...
    try:
        ...
        if data.booking_id:
            ...
            if booking:
                # Update booking status from HELD to ACTIVE
                booking.status = BookingStatus.ACTIVE
                booking.payment_status = PaymentStatus.PAID
                booking.transaction_id = data.razorpay_payment_id
                ...
                payment_tx = PaymentTransaction(
                    ...
                    method=PaymentMethod.UPI,  # Default, actual method can be updated
                ...
                # Update cabin status from RESERVED to OCCUPIED
                if booking.cabin_id:
                ...
                # Accounting shadow: compute tax + post ledger group.
                # Guarded internally by `accounting.enabled` and never raises.
                # Mirrors the call in routers/razorpay.py so every payment-
                # completion site freezes the same GST split.
                try:
...
async def confirm_venue_subscription(
    ...
    try:
        ...
        is_mock = data.payment_id.startswith("pay_mock_") or data.payment_id.startswith("pay_demo_")
        ...
        if not is_mock:
            ...
            if payment['status'] != 'captured':
             ...
             print(f"MOCK/DEMO SUBSCRIPTION: Skipping Razorpay fetch for {data.payment_id}")
...
async def get_owner_payment_history(
    ...
    """
    Get all payment transactions for owner's venues and accommodations.
    Includes:
    - Booking payments (initial & extensions) from reading rooms
    - Subscription payments from accommodations (PG/Hostel/House)
    """
    
    payment_list = []
    
    # ==================== PART 1: Reading Room Booking Payments ====================
    ...
    if owner_rooms:
        ...
        room_map = {r.id: r for r in owner_rooms}
        ...
        if cabin_ids:
            ...
            if booking_ids:
                ...
                # Also get bookings with PAID status but no PaymentTransaction (for backwards compatibility)
                paid_bookings_without_tx = [
                    b for b in owner_bookings 
                    if b.payment_status == PaymentStatus.PAID and 
                    b.id not in [p.booking_id for p in payments]
                ]
                
                for payment in payments:
                    booking = booking_map.get(payment.booking_id)
                    
                    # Skip payments without valid bookings (data inconsistency)
                    if not booking:
                        continue
                    
                    # Skip payments made by the owner themselves
                    if payment.user_id == current_user.id:
                        continue
                    
                    cabin = cabin_map.get(booking.cabin_id) if booking else None
                    room = room_map.get(cabin.reading_room_id) if cabin else None
                ...
                # Add legacy bookings (backwards compatibility for old paid bookings)
                for booking in paid_bookings_without_tx:
                    # Skip bookings made by the owner themselves
                    if booking.user_id == current_user.id:
                        continue
                    
                    cabin = cabin_map.get(booking.cabin_id)
                    room = room_map.get(cabin.reading_room_id) if cabin else None
                    ...
                    payment_list.append({
                        ...
                        "type": "INITIAL",
                        "amount": booking.amount,
                        "date": booking.created_at.isoformat() if booking.created_at else "",
                        "venue_name": room.name if room else "Unknown Venue",
                        "cabin_number": cabin.number if cabin else "N/A",
    ...
    # ==================== PART 2: Accommodation Subscription Payments ====================
    # NOTE: Excluded - accommodation subscription payments are owner's own expenses to list venues,
    # not customer revenue. Only showing customer booking payments in financial history.
...
@router.get("/admin/refunds/{refund_id}/credit-note/pdf")
async def get_refund_credit_note_pdf(
    refund_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Render the CREDIT_NOTE for an approved refund as a PDF.

    Available to the booking owner-of-record, the super admin, and the student
    whose refund it is. Returns 404 until a credit note has actually been
    issued (which requires `feature.credit_notes` to be on at approval time).
    """
    ...
    if cn is None:
        raise HTTPException(status_code=404, detail="Credit note not yet issued")

    # Access control: student-of-refund, super admin, or owner of the booking
    if current_user.role != UserRole.SUPER_ADMIN and current_user.id != refund.user_id:
    ...
    pdf_bytes = render_credit_note(
        credit_note_number=cn.invoice_number,
        original_invoice_number=original_invoice.invoice_number if original_invoice else None,
        issue_date=cn.generated_at,
        supplier=_party_dict_pay(supplier),
        recipient=_party_dict_pay(recipient),
    ...
    filename = cn.invoice_number.replace("/", "_") + ".pdf"
    return Response(
        content=pdf_bytes,
...
async def update_refund_status(
    ...
    # Issue credit note + reversing ledger on APPROVED / PROCESSED.
    # Guarded by `feature.credit_notes` flag — no-op until super admin opts in.
    if new_status in (RefundStatus.APPROVED, RefundStatus.PROCESSED):
        try:
            from app.services.credit_note_service import issue_for_refund
            cn = await issue_for_refund(db, refund_id=refund.id)
            if cn is not None:
                await db.commit()
        except Exception as cn_err:
            # Non-fatal: existing refund flow must keep working even if the
            # accounting layer fails. Integrity job will flag it.
            print(f"[credit_note] non-fatal: {cn_err}")

        # If the underlying booking was already settled, create a RECOVERY
        # charge so the next settlement run deducts the refund from the owner.
        # Recovery is also non-fatal: the cron-sweep in settlement_service
        # will catch any refund that slipped through.
        try:
            from app.services.recovery_service import create_recovery_charge_for_refund
            recovery = await create_recovery_charge_for_refund(db, refund_id=refund.id)
            if recovery is not None:
                await db.commit()
        except Exception as rec_err:
            print(f"[recovery] non-fatal: {rec_err}")
