...
async def update_accommodation(
    ...
    if acc.owner_id != current_user.id and current_user.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN]:
        raise HTTPException(status_code=403, detail="Not authorized")
...
async def verify_accommodation(
    ...
    # KYC gate — see reading_rooms.verify_reading_room for the same pattern.
    from app.routers.super_admin_kyc import KYCNotVerified, assert_owner_kyc_verified
    from app.models.user import KYCStatus, User as UserModel
    owner_kyc = (await db.execute(
        select(UserModel.kyc_status).where(UserModel.id == acc.owner_id)
    )).scalar_one_or_none()
    if owner_kyc not in (KYCStatus.VERIFIED, KYCStatus.NOT_REQUIRED):
        try:
            await assert_owner_kyc_verified(db, acc.owner_id)
