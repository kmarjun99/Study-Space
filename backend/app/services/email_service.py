...
MAIL_FROM = settings.mail_from or settings.mail_username or os.getenv("mail_from", "noreply@studyspace.com")
...
async def _send_email(to_email: str, subject: str, html_content: str):
    """Send email — tries SendGrid first, falls back to Gmail SMTP"""
    ...
    if sg:
        try:
            ...
            print(f"[EMAIL] SendGrid failed, falling back to SMTP: {e}")

    # Fall back to Gmail SMTP
    return await asyncio.to_thread(_send_smtp_sync, to_email, subject, html_content)
