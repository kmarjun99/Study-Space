...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    # Patterns for query parameters and body (includes SQL injection)
    SUSPICIOUS_PATTERNS = [
    ...
    # Patterns for paths only (excludes SQL keywords since they're common in REST APIs)
    PATH_PATTERNS = [
        r"<script[\s\S]*?>[\s\S]*?</script>",  # XSS
        r"javascript:",  # XSS
        r"on\w+\s*=",  # Event handlers
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        for key, value in request.query_params.items():
            if self._contains_suspicious_content(value, self.SUSPICIOUS_PATTERNS):
        ...
        # Check path parameters (using PATH_PATTERNS to allow REST API verbs)
        if self._contains_suspicious_content(request.url.path, self.PATH_PATTERNS):
    ...
    def _contains_suspicious_content(self, text: str, patterns: list) -> bool:
