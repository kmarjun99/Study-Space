...
export const ImageGallery: React.FC<ImageGalleryProps> = ({
    ...
    // Render images through the /img transform pipeline. The owner's raw
    // upload at /uploads/... is untouched; this only changes the URL we
    // ask the browser to fetch. External Unsplash/CDN URLs pass through
    // unchanged via `imageUrl`'s built-in guard.
    const getImageSrc = (index: number, width: number = 1200) => {
        if (imageErrors.has(index)) return FALLBACK_IMAGE;
        return imageUrl(displayImages[index], { w: width, fmt: 'webp' });
    };
        ...
        <>
            ...
            {isFullscreen && (
                ...
                >
                    ...
                    {/* Main Image — request a larger width when fullscreen */}
                    <img
                        src={getImageSrc(currentIndex, 1600)}
                        alt={`${venueName} - Photo ${currentIndex + 1}`}
                    ...
                    {hasMultipleImages && (
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3">
                            {displayImages.map((img, index) => (
                                ...
                                >
                                    <img
                                        src={imageErrors.has(index) ? FALLBACK_IMAGE : imageUrl(img, { w: 240, fmt: 'webp' })}
