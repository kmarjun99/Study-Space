...
export interface DurationPrice {
  [key: string]: number | null;  // e.g., { "1_DAY": 150, "1_WEEK": 900, "1_MONTH": 3000 }
}
...
export interface Booking {
  ...
  // Accounting layer fields populated by accounting_shadow at payment time.
  // All optional because legacy bookings (paid before accounting.enabled)
  // and bookings whose shadow failed silently have these as NULL.
  // The super-admin Bookings page reads these for the real GST-aware
  // financial breakdown instead of the legacy 90/10 hardcoded split.
  baseAmount?: number | null;       // taxable value (gross less GST)
  gstAmount?: number | null;        // GST owed on this booking
  gstRateApplied?: number | null;   // rate captured at payment (e.g. 0.18)
  gstTreatment?: string | null;     // OWNER_REGISTERED | SEC_9_5 | EXEMPT | NOT_REGISTERED | LEGACY | NOT_COMPUTED
  placeOfSupplyState?: string | null;  // 2-char state code
  paidAt?: string | null;           // ISO timestamp; used by settlement T+N gate
  settledAt?: string | null;        // non-null once a settlement run included this booking
  settlementRunId?: string | null;  // FK to the SettlementRun that paid this out
}
...
export interface SupportTicket {
  ...
  metaData?: {
    ...
    deviceInfo?: string;
    appVersion?: string;
    // Origin of the support request (e.g. "AdminDashboard",
    // "StudentPayments") for funneling analytics.
    source?: string;
  };
  adminNotes?: string;
