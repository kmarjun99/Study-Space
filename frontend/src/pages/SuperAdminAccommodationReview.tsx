                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {images.map((img: string, i: number) => (
                                    <div key={i} className="aspect-video bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                                        <img src={imageUrl(img, { w: 480, fmt: 'webp' })} alt={`Photo ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300 cursor-zoom-in" loading="lazy" decoding="async" />
                                    </div>
                                ))}
                            </div>
                        ) : (
