...
import { AppState, Cabin, CabinStatus, User, ReadingRoom, BookingDurationType } from '../types';
...
export const BookCabin: React.FC<BookCabinProps> = ({ state, user, onBookCabin, onJoinWaitlist }) => {
  ...
  // Filter ads for student carousel
  const studentAds = useMemo(() => {
    return ads.filter(ad => 
      ad.isActive && 
      (ad.targetAudience === 'STUDENT' || ad.targetAudience === 'ALL')
    );
  }, [ads]);

  // Get success ad (first from filtered ads, no need for useMemo with random function)
  const successAd = studentAds.length > 0 ? studentAds[0] : null;
  ...
  // Fire a VIEW event when a venue is selected for browsing.
  // Fails silently if the backend ingestion isn't enabled.
  useEffect(() => {
    if (selectedVenue) {
      track({
        event_name: 'view.reading_room',
        event_category: 'VIEW',
        entity_type: 'reading_room',
        entity_id: selectedVenue.id,
        city: selectedVenue.city || undefined,
        metadata: {
          venue_name: selectedVenue.name,
          price_start: selectedVenue.priceStart,
        },
      });
    }
  }, [selectedVenue?.id, track]);
  ...
  const handleCabinClick = (cabin: Cabin) => {
    ...
    if (cabin.status === CabinStatus.AVAILABLE) {
      ...
      if (selectedVenue) {
        const defaultDuration = getDefaultDuration(selectedVenue.allowedBookingDurations);
  ...
  const handlePayment = async () => {
    if (!selectedCabin || !selectedVenue) {
      console.error('[BOOKING] Missing required data:', { selectedCabin, selectedVenue });
      return;
    }

    if (!selectedCabin.id) {
      console.error('[BOOKING] Cabin ID is missing!', selectedCabin);
      alert('Error: Cabin ID is missing. Please try selecting the cabin again.');
      return;
    }

    console.log('[BOOKING] Starting payment process:', {
      cabin: selectedCabin,
      venue: selectedVenue,
    ...
    try {
      ...
      const durationPrice = getDurationPrice(selectedVenue.durationPrices, selectedDuration, selectedVenue.priceStart);
      if (!durationPrice) {
      ...
      // 1. Hold the cabin first (create booking). See ReadingRoomDetail.tsx
      // for the rationale — the previous raw-fetch + env-var pattern shipped
      // "http://localhost:8000" into the prod bundle, making every booking
      // ERR_CONNECTION_REFUSED on the live site.
      let booking: any;
      try {
      ...
      const totalAmount = booking.amount; // Use the amount calculated by backend
      ...
      if (orderData.is_demo || orderData.razorpay_key_id === 'demo_key_id' || orderData.razorpay_key_id === 'your_razorpay_key_id') {
        ...
        try {
          ...
          // FALLBACK — see ReadingRoomDetail.tsx for context. When verify
          // returns 500 but the booking is actually PAID server-side,
          // re-check the status and treat PAID as success.
      ...
      const options = {
        ...
        image: selectedVenue.imageUrl || '/logo.png',
  ...
  const renderVenueSelection = () => (
    <>
        ...
        <div className="flex-1 max-w-md">
          <LocationSearch
            ...
            onSelect={(location) => {
              ...
              // Phase 1 intelligence — fire a SEARCH event for funnel analysis.
              track({
                event_name: 'search.location',
                event_category: 'SEARCH',
                entity_type: 'search',
                city: location.city,
                location_query: location.display_name,
                metadata: { source: 'book-cabin' },
              });
            }}
            onClear={() => {
      ...
      {studentAds.length > 0 && (
        <div className="mb-6">
          <AdCarousel ads={studentAds} variant="banner" autoSlideInterval={5000} />
        </div>
      )}
            ...
            return (
              <Card key={venue.id} className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group" >
                <div className="h-48 overflow-hidden relative bg-gray-200">
                  <img
                    src={imageUrl(venue.imageUrl, { w: 640, fmt: 'webp' })}
                    alt={venue.name}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              {(() => {
                const availableDurations = getAvailableDurations(
                  selectedVenue.allowedBookingDurations,
                  selectedVenue.durationPrices,
                  selectedVenue.priceStart
                );
                ...
                return (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {availableDurations.map(durationType => {
                      const price = getDurationPrice(selectedVenue.durationPrices, durationType, selectedVenue.priceStart);
                      return (
                        <button
                          key={durationType}
            ...
            <div className="flex justify-between items-center pt-2">
              ...
              <span className="text-2xl font-bold text-gray-900">
                {formatPrice(getDurationPrice(selectedVenue.durationPrices, selectedDuration, selectedVenue.priceStart) || 0)}
              </span>
            </div>

            {/* GST breakdown — display-only. The Total Payable above is what
                actually gets charged; this just shows how much of it is tax.
                Fails soft if the backend preview endpoint is unavailable. */}
            <PriceBreakdownPreview
              listingType="reading-room"
              listingId={selectedVenue.id}
              displayedPrice={getDurationPrice(selectedVenue.durationPrices, selectedDuration, selectedVenue.priceStart) || 0}
              showNotes={false}
            />
