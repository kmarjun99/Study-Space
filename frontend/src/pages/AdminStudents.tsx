...
import { Search, Mail, Phone, FileText, User as UserIcon, Loader2 } from 'lucide-react';
...
export const AdminStudents: React.FC<AdminStudentsProps> = ({ state }) => {
  ...
  // Fetch students and bookings from API
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const [studentsData, bookingsData] = await Promise.all([
          venueService.getMyStudents(),
  ...
  // Bookings in this venue (from API)
  const venueBookings = bookings.filter(b => myCabinIds.has(b.cabinId));
