        ...
        <div className="space-y-6 max-w-6xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500">List and manage your PG, Hostel, or House properties.</p>
                </div>
                    ...
                    return (
                        <Card key={acc.id} className="overflow-hidden flex flex-col group relative">
                            ...
                            <div className="relative h-48 overflow-hidden bg-gray-100">
                                <img src={imageUrl(acc.imageUrl || images[0], { w: 640, fmt: 'webp' })} alt={acc.name} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                                <div className="absolute top-2 right-2 flex gap-1">
                                    <Badge variant={acc.type === 'PG' ? 'info' : acc.type === 'HOSTEL' ? 'warning' : 'success'}>{acc.type}</Badge>
                                </div>
                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity gap-2">
                                    <Button variant="secondary" size="sm" onClick={() => handleOpenEdit(acc)}>
                                        <Edit2 className="w-4 h-4 mr-1" /> Edit
                                    </Button>
                                    <Button
                                        variant="secondary"
                                        size="sm"
                                        onClick={() => navigate(`/admin/listings/accommodation/${acc.id}/billing`)}
                                        title="Billing &amp; Tax Settings"
                                    >
                                        <Wallet className="w-4 h-4 mr-1" /> Billing
                                    </Button>
                ...
                <div className="space-y-6">
                    ...
                    {wizardStep === 2 && (
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {images.map((img, i) => (
                                    <img key={i} src={imageUrl(img, { w: 240, fmt: 'webp' })} className="w-full h-24 object-cover rounded" loading="lazy" decoding="async" />
                                ))}
                    ...
                    {wizardStep === 3 && (
                        <div className="text-center space-y-6 py-8">
                            {!editingId ? (
                            ...
                            ) : formData.status === ListingStatus.VERIFICATION_PENDING || formData.status === ListingStatus.LIVE ? (
                                /* 🆕 Already Paid - Show Status Message */
                                <div className="max-w-md mx-auto">
                                    ...
                                    <h3 className="text-2xl font-bold text-gray-900 mb-2">
                                        {formData.status === ListingStatus.LIVE ? 'Listing is Live!' : 'Awaiting Approval'}
                                    </h3>
                                    <p className="text-gray-600 mb-4">
                                        {formData.status === ListingStatus.LIVE 
                                            ? 'Your property is live and visible to users!'
                                            : 'Your payment has been received. Your listing is being reviewed by our admin team.'
                                        }
                                    </p>
                                    ...
                                    {(state.subscriptionPlans || []).filter(p => p.isActive).length > 0 && (
                                        <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 mb-6">
                                            <p className="text-sm font-medium text-indigo-900 mb-2">Want to upgrade your plan?</p>
                                            <Button
                                                size="sm"
                                ...
                                /* Normal Payment Flow for DRAFT/REJECTED */
                                <div className="max-w-md mx-auto">
