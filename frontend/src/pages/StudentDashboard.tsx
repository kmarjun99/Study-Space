...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ 
    ...
    // Use notifications from global state instead of dummy data
    ...
    // IMPORTANT: Use useMemo to prevent creating a new object reference on every render
    const activeBooking = useMemo(() => {
        return activeBookings.length > 0 ? activeBookings[0] : null;
    }, [activeBookings.length > 0 ? activeBookings[0]?.id : null]);
        ...
        const checkReview = async () => {
            ...
            if (rrId) {
                ...
                try {
                    ...
                    setHasReviewed(prev => prev === reviewed ? prev : reviewed); // Only update if changed
                } catch (e) {
    ...
    }, [activeBooking?.id, activeBooking?.cabinId]); // Only depend on IDs, not entire objects
    ...
    }, [bookingHistory.length]); // Only depend on length, not array
    ...
    const activeCabin = useMemo(() => {
        ...
        const cabin = (state.cabins || []).find(c => c.id === activeBooking.cabinId);
        return cabin || null;
    }, [activeBooking?.cabinId]); // Only cabin ID needed
    ...
    const activeVenue = useMemo(() => {
        ...
        return venue || null;
    }, [activeCabin?.readingRoomId]); // Only reading room ID needed
    ...
    const venueToReview = useMemo(() => {
        ...
        return null;
    }, [activeVenue?.id, bookingHistory.length > 0 ? bookingHistory[0]?.id : null]); // Only IDs
    ...
    // Get Targeted Ads for Carousel
    const studentAds = useMemo(() => {
        // Filter ads for students
    ...
    // Get single ad for popup - call directly, don't memoize (getTargetedAd uses Math.random)
    const popupAd = studentAds.length > 0 ? studentAds[0] : null;
    ...
    // Notification handlers - now use props from parent
    ...
    const filteredBookings = useMemo(() => {
        ...
        return filtered;
    }, [activeBookings.length, statusFilter, searchQuery]); // Don't depend on state.cabins/readingRooms arrays
    ...
    return (
        ...
        <div className="space-y-4 md:space-y-8 max-w-5xl mx-auto">
            {/* Phase 3: rule-based recommendations. Rails hide themselves
                silently when consent is missing or the master flag is off,
                so we can mount them unconditionally. */}
            <RecommendationRail surface="for-me" limit={8} />
            <RecommendationRail surface="recently-viewed" limit={8} />
            <RecommendationRail surface="trending" limit={8} />
                                        ...
                                        {featuredVenues.map((venue, index) => (
                                            ...
                                            >
                                                ...
                                                <div className="relative h-40 w-full bg-gray-200 overflow-hidden">
                                                    <img
                                                        src={imageUrl(venue.imageUrl, { w: 480, fmt: 'webp' })}
                                                        alt={venue.name}
                ...
                {activeBooking && activeVenue ? (
                    <div className="space-y-6">
                        ...
                        <div className="h-32 w-full rounded-lg overflow-hidden bg-gray-200 relative">
                            <img
                                src={imageUrl(activeVenue.imageUrl, { w: 400, fmt: 'webp' })}
            ...
            >
                <div className="text-center py-4">
                    ...
                    <p className="text-gray-600 mb-6">
                        Thanks for using mySpace. As a valued student member, here is a special offer just for you.
                    </p>
