...
export const SuperAdminSupplyView = () => {
    ...
    const handleVerify = async (id: string, type: 'room' | 'accommodation') => {
        try {
            ...
            // Optimistic update:
            // Reading room schema uses alias → "isVerified" (camelCase)
            // Accommodation schema uses no alias → "is_verified" (snake_case)
            if (type === 'room') {
                setVenues(prev => prev.map(v => v.id === id ? { ...v, isVerified: true } : v));
            } else {
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center">
                                                    <div className="h-10 w-10 flex-shrink-0 mr-4 bg-gray-100 rounded-lg overflow-hidden">
                                                        {v.imageUrl ? <img src={imageUrl(v.imageUrl, { w: 240, fmt: 'webp' })} alt="" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <Building2 className="p-2 h-full w-full text-gray-400" />}
                                                    </div>
                                                    <div>
                                    ...
                                    accommodations.length > 0 ? accommodations.map(a => (
                                        <tr key={a.id} className="bg-white hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center">
                                                    <div className="h-10 w-10 flex-shrink-0 mr-4 bg-gray-100 rounded-lg overflow-hidden">
                                                        {a.imageUrl ? <img src={imageUrl(a.imageUrl, { w: 240, fmt: 'webp' })} alt="" className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <Building2 className="p-2 h-full w-full text-gray-400" />}
                                                    </div>
                                                    <div>
