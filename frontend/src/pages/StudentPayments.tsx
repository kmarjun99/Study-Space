...
// Format an ISO timestamp like "2026-06-01T07:59:38.063717" as
// "01 Jun 2026, 7:59 AM" — Indian conventions (en-IN), 12-hour clock so
// payment rows are scannable. Falls back to the raw string when the
// value can't be parsed (e.g. a half-constructed booking with no
// startDate) so the column never silently goes blank.
const formatPaymentDate = (raw: string | null | undefined): string => {
  if (!raw) return '—';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="p-6 flex items-center space-x-4">
          ...
          <div>
            ...
            {lastPayment && (
              <p className="text-xs text-gray-400">{formatPaymentDate(lastPayment.startDate)}</p>
            )}
          </div>
        </Card>
