...
const AdminVenueBase: React.FC<AdminVenueProps> = ({ state, onCreateRoom, onUpdateRoom, onAddCabin, onBulkAddCabins, onUpdateCabin, onBulkUpdateCabins, onBulkDeleteCabins }) => {
    ...
    const [batchConfig, setBatchConfig] = useState({
        floor: 1, startNum: 1, endNum: 20, amenities: ['WiFi', 'AC'] as string[], prefix: '', zone: 'MIDDLE' as 'FRONT' | 'MIDDLE' | 'BACK'
    });
    
    // Duration configuration state
    const [enabledDurations, setEnabledDurations] = useState<string[]>(['1_MONTH']);
    const [durationPrices, setDurationPrices] = useState<{[key: string]: number}>({
        '1_MONTH': venueFormData.priceStart || 0
    });
    ...
    // --- Tab State ---
    const [activeTab, setActiveTab] = useState<'details' | 'photos' | 'cabins' | 'waitlist' | 'durations'>('details');
    ...
    // Sync base price with 1_MONTH duration price
    useEffect(() => {
        if (venueFormData.priceStart && venueFormData.priceStart > 0) {
            setDurationPrices(prev => ({
                ...prev,
                '1_MONTH': venueFormData.priceStart!
            }));
        }
    }, [venueFormData.priceStart]);
    ...
    const handleProceedToPayment = () => {
        // 🆕 Check if already paid and awaiting approval
        if (venue?.status === ListingStatus.VERIFICATION_PENDING || venue?.status === ListingStatus.LIVE) {
            if (venue.subscriptionPlanId) {
                // Has a plan - can upgrade
                setIsPaymentModalOpen(true);
            } else {
                // Paid but no plan set - awaiting approval
                alert("Your venue is awaiting admin approval. Payment has already been completed.");
                return;
            }
        } else {
            // New payment flow
    ...
    // Check for any existing boost request for this venue
    const existingRequest = myBoostRequests.find(
        r => r.venueId === venue?.id && 
        ['initiated', 'payment_pending', 'paid', 'admin_review', 'approved', 'active'].includes(r.status)
    );

    // Check if the existing request can be cancelled
    const canCancelRequest = existingRequest && ['initiated', 'payment_pending'].includes(existingRequest.status);

    const handleCancelBoostRequest = async () => {
        if (!existingRequest) return;
        
        try {
            await boostService.cancelRequest(existingRequest.id);
            toast.success('Boost request cancelled successfully');
            // Refresh the boost requests list
            const updatedRequests = await boostService.getMyRequests();
            setMyBoostRequests(updatedRequests);
        } catch (error) {
            console.error('Failed to cancel boost request:', error);
    ...
    const handleBoostSubmit = async () => {
        ...
        try {
            ...
            const orderData = await paymentService.createBoostOrder(newRequest.id, selectedPlan.price);
            ...
            const errorMessage = (error as any)?.response?.data?.detail || 
                                ...
                                'Failed to initiate boost. Please try again.';
    ...
    const renderTabs = () => (
        <div className="flex border-b border-gray-200 mb-6 bg-white sticky top-0 z-10">
            ...
            <button
                ...
                className={`flex items-center px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'durations' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'
                    }`}
            >
                Duration Settings
            </button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                {displayImages.map((img, idx) => (
                    <div key={idx} className="relative aspect-video rounded-lg overflow-hidden border group">
                        <img src={imageUrl(img, { w: 480, fmt: 'webp' })} alt={`Venue ${idx}`} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                        {!isReadOnly && (
    ...
    const renderDurationConfigForm = () => {
        const durationTypes = [
        ...
        const handleToggleDuration = (durationType: string) => {
            if (enabledDurations.includes(durationType)) {
                // Remove if not the last one
                if (enabledDurations.length > 1) {
                    setEnabledDurations(prev => prev.filter(d => d !== durationType));
                } else {
                    toast.error('At least one booking duration must be enabled');
                }
            } else {
                setEnabledDurations(prev => [...prev, durationType]);
            }
        };

        const handlePriceChange = (durationType: string, value: string) => {
        ...
        const handleSaveDurationConfig = async () => {
            // Validate
            const missingPrices = enabledDurations.filter(d => !durationPrices[d] || durationPrices[d] <= 0);
            if (missingPrices.length > 0) {
                toast.error('Please set valid prices for all enabled durations');
                return;
            }

            if (!venue?.id) {
                toast.error('Please save venue details first');
                return;
            }
            ...
            try {
                ...
                console.error('Failed to save duration config:', error);
        ...
        return (
            <Card className="p-6">
                ...
                <p className="text-sm text-gray-500 mb-6">
                    Configure which booking durations are available and set pricing for each option.
                </p>

                <div className="space-y-4">
                    {durationTypes.map((durationType) => {
                        const isEnabled = enabledDurations.includes(durationType.value);
                        const price = durationPrices[durationType.value] || 0;
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            <input
                                                type="checkbox"
                                                checked={isEnabled}
                                                onChange={() => handleToggleDuration(durationType.value)}
                                            ...
                                            <div className="ml-7 flex items-center gap-2">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        type="number"
                                                        value={price || ''}
                                                        onChange={(e) => handlePriceChange(durationType.value, e.target.value)}
                                                        placeholder="0"
                                                        className="pl-7 pr-3 py-1.5 border border-gray-300 rounded-md w-32 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                                        min="0"
                                                        step="1"
                                                    />
                                                </div>
                ...
                <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-800">
                        💡 <strong>Tip:</strong> Select at least one duration type. Students will only see the durations you enable here.
                    </p>
                </div>
                ...
                <div className="mt-6 flex justify-between">
                    ...
                    >
                        Save & Submit for Payment <ArrowRight className="w-4 h-4 ml-2" />
                    </Button>
                </div>
            </Card>
        );
    ...
    const renderSharedModals = () => (
        <>
            <Modal isOpen={isCabinModalOpen} onClose={() => setIsCabinModalOpen(false)} title={editingCabinId ? "Edit Cabin" : "Add Cabin"}>
                <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        <p className="text-sm text-blue-800">
                            <strong>Price:</strong> Automatically synced with venue's base price (₹{venue?.priceStart || 0}/month). Configure duration-specific prices in the Duration Settings tab.
                        </p>
                    </div>
            ...
            <Modal isOpen={isBatchModalOpen} onClose={() => setIsBatchModalOpen(false)} title="Batch Create Cabins">
                <div className="space-y-4">
                    <Input label="Start Number" type="number" value={batchConfig.startNum} onChange={(e: any) => setBatchConfig({ ...batchConfig, startNum: Number(e.target.value) })} />
                    <Input label="End Number" type="number" value={batchConfig.endNum} onChange={(e: any) => setBatchConfig({ ...batchConfig, endNum: Number(e.target.value) })} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2">
                        ...
                        <div className="mt-4">
                            ...
                            <div className={activeTab === 'durations' ? 'block' : 'hidden'}>
                                {venue && <DurationSettings readingRoom={venue} onUpdate={(updated) => setFetchedVenue(updated)} />}
                            </div>
                    ...
                    <div>
                        ...
                        <Card className="p-4 mt-6">
                            ...
                            <Button
                                className="w-full mb-2"
                                variant="outline"
                                onClick={() => venue?.id && navigate(`/admin/listings/reading-room/${venue.id}/billing`)}
                                disabled={!venue?.id}
                            >
                    ...
                    {boostSuccess ? (
                        ...
                        <div className="text-center py-8">
                            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${
                                existingRequest.status === 'active' || existingRequest.status === 'approved' 
                                    ? 'bg-green-100' 
                                    : existingRequest.status === 'payment_pending' || existingRequest.status === 'initiated'
                                    ? 'bg-orange-100'
                                    : 'bg-yellow-100'
                            }`}>
                                {existingRequest.status === 'active' || existingRequest.status === 'approved' ? (
                            ...
                            <h3 className="text-lg font-bold text-gray-900 mb-2">
                                {existingRequest.status === 'active' || existingRequest.status === 'approved' 
                                    ...
                                    ? 'Payment Pending'
                                    : existingRequest.status === 'initiated'
                                    ? 'Request Initiated'
                            ...
                            <p className="text-gray-600 mb-4">
                                {existingRequest.status === 'active' || existingRequest.status === 'approved'
                                    ...
                                    : existingRequest.status === 'payment_pending'
                                    ? 'You have an incomplete payment for this venue. Complete the payment or cancel to create a new request.'
                                    : existingRequest.status === 'initiated'
                                    ? 'You have an initiated request. Complete it or cancel to start fresh.'
                                    : existingRequest.status === 'paid'
                                    ? 'Your payment is confirmed. Awaiting Super Admin approval.'
                            ...
                            {canCancelRequest && (
                                <div className="mt-6">
                                    <Button
                                        ...
                                        onClick={handleCancelBoostRequest}
                                    >
                                        Cancel This Request
                                    </Button>
                                    <p className="text-xs text-gray-500 mt-2">You can cancel and create a new request</p>
                                </div>
                            )}
                        </div>
                    ) : (
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 pb-24">
            ...
            {step === 4 && renderDurationConfigForm()}
