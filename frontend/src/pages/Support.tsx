...
export const SupportPage: React.FC<SupportProps> = ({ user, tickets, onTicketCreate }) => {
    ...
    // Live "my tickets" list fetched from the backend (overrides the prop
    // version once it has loaded). The prop is still respected as the
    // initial value so the page renders something during the first paint.
    const [myTicketsState, setMyTicketsState] = useState<SupportTicket[] | null>(null);
    ...
    // Fetch the user's own tickets on first switch to MY_TICKETS (and on mount
    // so the badge counts/UX stays in sync). Falls back to the tickets prop if
    // the fetch fails or hasn't completed yet.
    React.useEffect(() => {
        loadMyTickets();
    }, [loadMyTickets]);

    const sourceTickets: SupportTicket[] = myTicketsState ?? tickets;
    const myTickets = sourceTickets
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Refresh the live "my tickets" list so the new ticket shows up
            // even if the parent doesn't sync the prop.
            loadMyTickets();
                ...
                <div className="space-y-4">
                    {myTicketsLoading && myTickets.length === 0 && (
                        <div className="flex items-center justify-center py-12">
                            ...
                            <span className="ml-3 text-gray-500 text-sm">Loading your tickets...</span>
                        </div>
                    )}
                    ...
                    {myTicketsError && !myTicketsLoading && (
                        <Card className="p-5 border-red-200 bg-red-50">
                            <div className="flex flex-col items-center text-center gap-3">
                                <p className="text-red-700 text-sm">{myTicketsError}</p>
                                <Button size="sm" variant="primary" onClick={loadMyTickets}>Retry</Button>
                            </div>
                        </Card>
                    )}
                    ...
                    {!myTicketsError && myTickets.length > 0 && (
