...
export const AdminFinancials: React.FC<AdminFinancialsProps> = ({ state }) => {
   ...
   // Defer Recharts mount until after the first paint so the parent <div>
   // has resolved its Tailwind h-72 height. Without this, Recharts measures
   // a 0-height container and logs "width(-1) and height(-1)" warnings.
   ...
   const transactions = useMemo(() => {
      if (paymentHistory.length > 0) {
         // API payments are all successful/PAID (no status field in API)
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-3">
               <Card className="p-6">
                  ...
                  <div className="h-72 w-full min-w-0" style={{ minHeight: 288, minWidth: 100 }}>
                     {chartMounted && (
                     ...
                     {isLoading ? (
                        ...
                        filteredTransactions.map((t) => (
                           <tr key={t.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                                 <button 
                                    onClick={() => handleDownloadInvoice(t.booking_id)}
                                    disabled={downloadingInvoice === t.booking_id}
                                    className="text-indigo-600 hover:text-indigo-900 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                                 >
                                    {downloadingInvoice === t.booking_id ? 'Downloading...' : 'View Invoice'}
                                 </button>
                              </td>
                           </tr>
                        ))
                        ...
                        <tr>
                           <td colSpan={6} className="px-6 py-10 text-center text-gray-500">
                              ...
                              {transactions.length === 0 && (
                                 <p className="text-xs mt-1">Transactions will appear here once students make bookings</p>
                              )}
                           </td>
                        </tr>
                     )}
                     ...
                     <div key={t.id} className="p-4 bg-white space-y-3">
                        ...
                        <div className="pl-12 pt-2 flex items-center justify-between border-t border-gray-50 mt-2">
                           ...
                           <button 
                              onClick={() => handleDownloadInvoice(t.booking_id)}
                              disabled={downloadingInvoice === t.booking_id}
                              className="text-indigo-600 text-sm font-medium hover:bg-indigo-50 px-2 py-1 rounded-md transition-colors flex items-center mt-1 disabled:opacity-50 disabled:cursor-not-allowed"
                           >
                              {downloadingInvoice === t.booking_id ? 'Downloading...' : 'View Invoice'}
                           </button>
                        </div>
