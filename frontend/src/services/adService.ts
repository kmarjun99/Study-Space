...
export const adService = {
  ...
  createAd: async (ad: Partial<Ad>): Promise<Ad> => {
    ...
    const responseAd = response.data;
    // Map snake_case response to camelCase
    return {
      id: responseAd.id,
      title: responseAd.title,
      description: responseAd.description,
      imageUrl: responseAd.image_url,
      ctaText: responseAd.cta_text,
      link: responseAd.link,
      categoryId: responseAd.category_id,
      targetAudience: responseAd.target_audience,
      isActive: responseAd.is_active,
      createdAt: responseAd.created_at,
      startDate: responseAd.start_date,
      endDate: responseAd.end_date,
    };
  },
  ...
  // Fix blob URLs (Super Admin only)
  fixBlobUrls: async (): Promise<{
    message: string;
    fixed_count: number;
    ads_fixed: Array<{ id: string; title: string; old_url: string; new_url: string }>;
    next_steps: string;
  }> => {
    const response = await api.post('/ads/fix-blob-urls');
    return response.data;
  },
