...
export const venueService = {
    ...
    getAllReadingRooms: async (): Promise<ReadingRoom[]> => {
        const response = await api.get('/api/reading-rooms/');
        // Safety check
        ...
        return response.data.map((room: any) => ({
            ...
            ownerId: room.ownerId || room.owner_id,
            priceStart: room.priceStart || room.price_start,
    ...
    deleteReadingRoom: async (roomId: string): Promise<void> => {
        await api.delete(`/api/reading-rooms/${roomId}`);
    },

    getReadingRoomById: async (roomId: string): Promise<ReadingRoom> => {
        const response = await api.get(`/api/reading-rooms/${roomId}`);
    ...
    getAllCabins: async (): Promise<Cabin[]> => {
        ...
        return response.data.map((cabin: any) => ({
            ...cabin,
            // Backend now returns camelCase via aliases, but handle both for backwards compatibility
    ...
    createReadingRoom: async (data: Partial<ReadingRoom>): Promise<ReadingRoom> => {
        ...
        const response = await api.post('/api/reading-rooms/', payload);
        const room = response.data;
        return {
            ...room,
            imageUrl: room.imageUrl || room.image_url,
    ...
    submitVenuePayment: async (id: string): Promise<void> => {
        await api.put(`/api/reading-rooms/${id}/submit-payment`);
    },

    verifyVenue: async (id: string): Promise<ReadingRoom> => {
        const response = await api.put(`/api/reading-rooms/${id}/verify`);
        const room = response.data;
        return {
            ...room,
            imageUrl: room.imageUrl || room.image_url,
    ...
    getMyStudents: async (): Promise<any[]> => {
        const response = await api.get('/api/reading-rooms/my-students');
    ...
    createCabin: async (readingRoomId: string, data: Partial<Cabin>): Promise<Cabin> => {
        const payload = {
            number: data.number,
            floor: data.floor,
            price: data.price || undefined, // Optional - backend will use venue's priceStart if not provided
            status: data.status || 'AVAILABLE',
            amenities: data.amenities || [], // Send List
            zone: data.zone,
            row_label: data.rowLabel
        };
        const response = await api.post(`/api/reading-rooms/${readingRoomId}/cabins`, payload);
    ...
    updateCabin: async (cabinId: string, data: Partial<Cabin>): Promise<Cabin> => {
        ...
        const response = await api.put(`/api/cabins/${cabinId}`, payload);
        const cabin = response.data;
        return {
            ...cabin,
            readingRoomId: cabin.readingRoomId || cabin.reading_room_id,
    ...
    deleteCabins: async (cabinIds: string[]): Promise<void> => {
        await api.post('/api/cabins/bulk-delete', { cabin_ids: cabinIds });
    },
    ...
    updateCabinsPrices: async (cabinIds: string[], newPrice: number): Promise<void> => {
        ...
        const promises = cabinIds.map(id =>
            api.put(`/api/cabins/${id}`, { price: newPrice })
        );
